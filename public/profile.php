...
$fetch = $pdo->prepare('SELECT id, username, email, display_name, bio, is_public, theme, created_at FROM users WHERE id = ? LIMIT 1');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($profile['username'] === '' || strlen($profile['username']) > 30) $errors[] = 'Username must be between 1 and 30 characters.';
    ...
    $duplicate->execute([$profile['username'], $profile['email'], $_SESSION['user_id']]);
    if ($duplicate->fetch()) $errors[] = 'That username or email is already in use.';
    ...
    if (!$errors) {
        $sql = 'UPDATE users SET username=?, display_name=?, email=?, bio=?, is_public=?, theme=?';
        $values = [$profile['username'], $profile['display_name'] ?: null, $profile['email'], $profile['bio'] ?: null, $profile['is_public'], $profile['theme']];
        if ($passwordHash !== null) { $sql .= ', password=?'; $values[] = $passwordHash; }
        $sql .= ' WHERE id=?'; $values[] = $_SESSION['user_id'];
        $pdo->prepare($sql)->execute($values);
        $_SESSION['username'] = $profile['username']; $_SESSION['email'] = $profile['email']; $_SESSION['theme'] = $profile['theme'];
...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Profile & settings | Plantliotecha</title><link rel="stylesheet" href="assets/css/app.css"><link rel="stylesheet" href="assets/css/theme.css"></head><body data-theme="<?= htmlspecialchars($theme) ?>"><main class="container profile-page">
<p><a href="dashboard.php">&larr; Dashboard</a> &middot; <a href="plants.php">My plants</a> &middot; <a href="logout.php">Log out</a></p>
<header class="profile-header"><div class="profile-avatar" aria-hidden="true"><?= htmlspecialchars($initial) ?></div><div><h1>Profile & settings</h1><p>Personalize your Plantliotecha account and privacy preferences.</p></div></header>
